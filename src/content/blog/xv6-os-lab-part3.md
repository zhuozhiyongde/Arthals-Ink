...
由于发现原先的 Makefile 不能立刻应用 `xv6-user/init.c` 中的修改，于是重新修了一下 Makefile，得到如下配置：
...
这里还发现一个之前说错的地方，现在使用 `gen_initcode.sh` 实际上是直接将整个 `init` 程序封到了 `initcode` 里，而不是先前说的启动 `/init` 了。

同时，为了 Debug，我还引进了一个新的 `recompile_test.sh`，这个代码可以在修改测试用例（比如添加打印）的源代码后，直接重新打包 `riscv64` 目录到当前文件夹，不过如果你要使用，可能需要微调下相关路径：
...
clone(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // 如果用户提供了栈地址
  if (stack != NULL) {
    // 根据测试用例的约定，从栈上读取 fn 和 arg
    // fn (函数指针) 存放在 stack 指向的位置
    uint64 fn = *((uint64*)stack);
    // arg (参数) 存放在 stack + 8 的位置
    uint64 arg = *((uint64*)((char*)stack + 8));

    np->trapframe->sp = stack; // 设置子进程的栈指针
    np->trapframe->epc = fn;   // 设置子进程的程序计数器，让它从 fn 开始执行
    np->trapframe->a1 = arg;   // 设置 a1 寄存器，作为 fn 的参数
  }
