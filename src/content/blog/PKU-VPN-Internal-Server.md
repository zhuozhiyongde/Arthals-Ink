...
首先登录 Tailscale，随后需要关闭 Tailscale DNS：
...
对于 GUI 版本，可以在 `Settings > General` 中取消勾选 `Use Tailscale DNS settings`。
...
其中，`100.255.255.255` 是校内服务器在 Tailscale 上组网的内网 IP 地址，而 `your tailscale interface` 为你的 Tailscale 隧道接口，其可以通过如下命令获得（注意初次获取的时候不要开 TUN / 增强模式）：
...
- macOS：
  ...
  你应该会得到一个形如 `interface: utun7` 的输出，而其中的 `utun7` 就是隧道接口。

- Windows：默认是 `Tailscale`

- Linux：默认是 `tailscale0`

注意这个接口名称在 macOS 下会随着电脑重启或者 Tailscale 开关而发生变化，如果你发现不能正常代理了，可以尝试重新获取一下（所以建议使用后文的覆写操作，好改）。
...
注意，开启增强模式时，不推荐同时开启系统代理，尽管多数情况下这表现良好。但倘若你在浏览器中对一个 TailScale 内网 IP 发起请求时，会由于其首先被系统代理接管，然后由于其 DNS 无法处理 `100.x.y.z` 之类的内网 IP 地址（准确为 `100.64.0.0/10` 网段），从而导致请求失败。而若只开启增强模式不开启系统代理，其会被 Tailscale 直接接管，从而能正确处理。
...
- [ChrAlpha / Tailscale 配合 Mihomo(Clash.Meta) TUN/Quantumult X VPN 共存使用技巧](https://blog.ichr.me/post/tailscale-mihomo-quantumult-x/)
